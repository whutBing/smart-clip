#pragma once

#include <map>
#include <string>
#include <vector>

// 内置语言（代码中硬编码）
enum AppLanguage {
  LANG_ZH_CN = 0,
  LANG_EN_US = 1,
  // 外部语言（通过 lang/*.ini 文件加载）
  LANG_JA_JP = 2,
  LANG_KO_KR = 3,
  LANG_DE_DE = 4,
  LANG_AR_SA = 5,
  LANG_TR_TR = 6,
  LANG_HI_IN = 7,
  LANG_BG_BG = 8,
  LANG_ES_VE = 9,
  LANG_TH_TH = 10,
  LANG_COUNT
};

enum StringId {
  STR_APP_TITLE = 0,
  STR_SETTINGS_TITLE,
  STR_SETTINGS_GENERAL,
  STR_SETTINGS_HOTKEY,
  STR_SETTINGS_DATA,
  STR_SETTINGS_SMART_ACTION,
  STR_SETTINGS_GENERAL_DESC,
  STR_SETTINGS_HOTKEY_DESC,
  STR_SETTINGS_SMART_ACTION_DESC,
  STR_ROW_NOTIFICATION,
  STR_ROW_NOTIFICATION_DESC,
  STR_ROW_SMOOTH_SCROLL,
  STR_ROW_SMOOTH_SCROLL_DESC,
  STR_ROW_SCROLLBAR,
  STR_ROW_SCROLLBAR_DESC,
  STR_ROW_SCROLLBAR_DELAY,
  STR_ROW_SCROLLBAR_DELAY_DESC,
  STR_ROW_THEME_MODE,
  STR_ROW_THEME_MODE_DESC,
  STR_ROW_THEME_STYLE,
  STR_ROW_THEME_STYLE_DESC,
  STR_ROW_LANGUAGE,
  STR_ROW_LANGUAGE_DESC,
  STR_ROW_IMAGE_PREVIEW,
  STR_ROW_IMAGE_PREVIEW_DESC,
  STR_ROW_TASKBAR,
  STR_ROW_TASKBAR_DESC,
  STR_ROW_HISTORY_LIMIT,
  STR_ROW_HISTORY_LIMIT_DESC,
  STR_ROW_HOTKEY_TOGGLE,
  STR_ROW_HOTKEY_TOGGLE_DESC,
  STR_ROW_HOTKEY_SEARCH,
  STR_ROW_HOTKEY_SEARCH_DESC,
  STR_ROW_QUICK_PASTE,
  STR_ROW_QUICK_PASTE_DESC,
  STR_ROW_QUICK_PASTE_MOD,
  STR_ROW_QUICK_PASTE_MOD_DESC,
  STR_ROW_FAVORITE_HOTKEY,
  STR_ROW_FAVORITE_HOTKEY_DESC,
  STR_ROW_DATA_SIZE,
  STR_ROW_PASTE_COUNT,
  STR_ROW_SET_DATA_DIR,
  STR_ROW_CLEAR_NON_FAV,
  STR_ROW_CLEAR_NON_FAV_DESC,
  STR_ROW_CLEAN_INVALID_IMAGES,
  STR_ROW_CLEAN_INVALID_IMAGES_DESC,
  STR_ROW_EXPORT_DATA,
  STR_ROW_EXPORT_DATA_DESC,
  STR_ROW_TEXT_SIZE_LIMIT,
  STR_ROW_TEXT_SIZE_LIMIT_DESC,
  STR_THEME_LIGHT,
  STR_THEME_DARK,
  STR_THEME_SYSTEM,
  STR_THEME_STYLE_CLASSIC,
  STR_THEME_STYLE_GRAPHITE,
  STR_THEME_STYLE_WARM,
  STR_THEME_STYLE_HIGH_CONTRAST,
  STR_LANGUAGE_ZH_CN,
  STR_LANGUAGE_EN_US,
  STR_LANGUAGE_JA_JP,
  STR_LANGUAGE_KO_KR,
  STR_LANGUAGE_DE_DE,
  STR_LANGUAGE_AR_SA,
  STR_LANGUAGE_TR_TR,
  STR_LANGUAGE_HI_IN,
  STR_LANGUAGE_BG_BG,
  STR_LANGUAGE_ES_VE,
  STR_LANGUAGE_TH_TH,
  STR_PREVIEW_OFF,
  STR_PREVIEW_BLUR,
  STR_PREVIEW_SD,
  STR_PREVIEW_HD,
  STR_FILTER_ALL,
  STR_FILTER_TEXT,
  STR_FILTER_IMAGE,
  STR_FILTER_FILE,
  STR_FILTER_FAVORITE,
  STR_SEARCH_PLACEHOLDER,
  STR_BTN_PIN,
  STR_BTN_UNPIN,
  STR_BTN_BATCH_EDIT,
  STR_BTN_DARKMODE,
  STR_TOOLTIP_PIN,
  STR_TOOLTIP_BATCH_EDIT,
  STR_TOOLTIP_DARKMODE,
  STR_TOOLTIP_PREV_PAGE,
  STR_TOOLTIP_NEXT_PAGE,
  STR_TOOLTIP_FAVORITE_FILTER,
  STR_TOOLTIP_MINIMIZE,
  STR_TOOLTIP_MAXIMIZE,
  STR_TOOLTIP_CLOSE,
  STR_TRAY_HINT_TITLE,
  STR_TRAY_MENU_SETTINGS,
  STR_TRAY_MENU_NOTIFICATIONS,
  STR_TRAY_MENU_LIGHT,
  STR_TRAY_MENU_DARK,
  STR_TRAY_MENU_EXIT,
  STR_TRAY_MENU_RESTART,
  STR_TRAY_MENU_VIEW_AGREEMENT, // "查看用户协议"
  STR_TRAY_NOTIFY_UPDATED,
  STR_TRAY_NOTIFICATIONS_ENABLED,
  STR_TRAY_TIP,
  STR_TRAY_PASTED,
  STR_SETTINGS_ABOUT,
  STR_SETTINGS_ABOUT_DESC,
  STR_ROW_STARTUP,
  STR_ROW_STARTUP_DESC,
  STR_ROW_VERSION,
  STR_ROW_VERSION_DESC,
  STR_ROW_CREDITS,
  STR_ROW_CREDITS_DESC,
  STR_CREDITS_TOOLTIP,
  STR_ROW_AGREEMENT,
  STR_ROW_AGREEMENT_DESC,
  STR_EULA,
  STR_PRIVACY_POLICY,
  STR_AGREEMENT_NOTE,
  STR_EULA_TITLE,
  STR_EULA_BODY,
  STR_PRIVACY_TITLE,
  STR_PRIVACY_BODY,

  STR_TRAY_FAVORITE_HOTKEY,
  STR_PASTE_COUNT_SUFFIX,
  STR_BTN_SELECT,
  STR_BTN_CLEAN,
  STR_BTN_EXPORT,
  STR_ROW_IMPORT_DATA,
  STR_ROW_IMPORT_DATA_DESC,
  STR_BTN_IMPORT,
  STR_DATA_BACKUP_TITLE,
  STR_DATA_BACKUP_DESC,
  STR_BTN_CONFIRM_CLEAR,
  STR_DLG_SELECT_DATA_DIR,
  STR_DLG_CLEAR_NON_FAV_TITLE,
  STR_DLG_CLEAR_NON_FAV_MSG,
  STR_DLG_CLEAN_INVALID_IMAGES_TITLE,
  STR_DLG_CLEAN_INVALID_IMAGES_MSG,
  STR_TRAY_HINT,
  STR_TRAY_CLEARED_NON_FAV,
  STR_TRAY_CLEARED_INVALID_IMAGES,

  STR_DLG_CONFIRM_MIGRATE_TITLE,
  STR_DLG_CONFIRM_MIGRATE_MSG,
  STR_DLG_CONFIRM_CANCEL,

  STR_DLG_CLEAR_NON_FAV_SUBTITLE,
  STR_DLG_CLEAR_NON_FAV_SUB_DESC,
  STR_DLG_CLEAR_NON_FAV_DESC,

  STR_DLG_CLEAN_INVALID_IMAGES_SUBTITLE,
  STR_DLG_CLEAN_INVALID_IMAGES_SUB_DESC,
  STR_DLG_CLEAN_INVALID_IMAGES_DESC,

  STR_MOD_CTRL,
  STR_MOD_ALT,
  STR_MOD_SHIFT,
  STR_MOD_WIN,
  STR_MOD_NONE,
  STR_MOD_CTRL_ALT,
  STR_MOD_CTRL_SHIFT,
  STR_MOD_ALT_SHIFT,
  STR_MOD_CTRL_ALT_SHIFT,

  // 右键菜单
  STR_CTX_COPY,
  STR_CTX_PASTE,
  STR_CTX_EDIT,
  STR_CTX_TAG,
  STR_CTX_OPEN_LOCATION,
  STR_CTX_DELETE,
  STR_CTX_BATCH_ADD_TAG,
  STR_CTX_BATCH_DELETE,
  STR_CTX_NO_TAGS,

  // 删除收藏确认对话框
  STR_DLG_DELETE_FAV_TITLE,
  STR_DLG_DELETE_FAV_SUBTITLE,
  STR_DLG_DELETE_FAV_BODY1,
  STR_DLG_DELETE_FAV_BODY2,
  STR_DLG_DELETE_FAV_CONFIRM,

  // 批量操作通知
  STR_TRAY_BATCH_DELETED,
  STR_TRAY_BATCH_TAGGED,

  // 快捷键占位文本
  STR_HOTKEY_PLACEHOLDER,

  // GitHub 仓库链接
  STR_GITHUB_REPO,

  // 通知消息
  STR_TRAY_NEW_CONTENT,
  STR_TRAY_COPY_TITLE,
  STR_TRAY_IMAGE_COPIED,
  STR_TRAY_FILE_PATH_COPIED,
  STR_TRAY_IMAGE_ADDED,
  STR_TRAY_DELETED,
  STR_TRAY_COPIED,
  STR_TRAY_PINNED,
  STR_TRAY_UNPINNED,
  // 多文件记录显示与图片预览
  STR_MULTI_FILES_FMT,        // "等 %d 个文件"
  STR_IMAGE_PREVIEW_TITLE,    // "图像预览 - 点击或按ESC关闭"
  STR_TEXT_PREVIEW_TITLE,     // "文本预览 - 空格/Esc/点击关闭"
  STR_CTX_SELECT_IN_EXPLORER, // "在资源管理器中选中"
  STR_TRAY_QUICK_PASTE_TITLE,
  STR_TRAY_HOTKEY_SETTINGS,
  STR_TRAY_SETTINGS_UPDATED,
  STR_TRAY_HOTKEY_SAVED,
  STR_TRAY_HOTKEY_FAILED,
  STR_TRAY_NOTIFY_ENABLED,
  STR_TOOLTIP_FILTER_BY_APP,
  STR_TOOLTIP_FILTER_BY_DATE,

  // 托盘暂停/恢复
  STR_TRAY_MENU_PAUSE,
  STR_TRAY_MENU_RESUME,
  STR_TRAY_PAUSED,
  STR_TRAY_RESUMED,

  // 托盘启用/关闭快捷键
  STR_TRAY_MENU_QUICK_PASTE_ENABLE,
  STR_TRAY_MENU_QUICK_PASTE_DISABLE,
  STR_TRAY_QUICK_PASTE_ENABLED,
  STR_TRAY_QUICK_PASTE_DISABLED,

  // 首次运行用户协议弹窗
  STR_AGREEMENT_DIALOG_TITLE,
  STR_AGREEMENT_DIALOG_SUBTITLE,
  STR_AGREEMENT_DIALOG_BODY,
  STR_AGREEMENT_DIALOG_ACCEPT,
  STR_AGREEMENT_DIALOG_DECLINE,
  STR_AGREEMENT_DIALOG_REMIND,

  // 悬浮选中（通用设置）
  STR_ROW_HOVER_SELECT,
  STR_ROW_HOVER_SELECT_DESC,

  // 本机信息（关于页）
  STR_ROW_MACHINE_INFO,
  STR_ROW_MACHINE_INFO_DESC,
  STR_MACHINE_INFO_TITLE,
  STR_MACHINE_INFO_CODE_LABEL,
  STR_MACHINE_INFO_COPY,
  STR_MACHINE_INFO_COPIED,
  STR_MACHINE_INFO_HW_LABEL,
  STR_MACHINE_INFO_COMPUTER,
  STR_MACHINE_INFO_OS,
  STR_MACHINE_INFO_CPU,
  STR_MACHINE_INFO_RAM,
  STR_MACHINE_INFO_DISK,

  STR_COUNT
};

extern AppLanguage g_appLanguage;

const wchar_t *T(StringId id);
void ApplyLanguage();
std::wstring GetLanguageCode(AppLanguage lang);

// 外部语言文件支持
struct LanguageInfo {
  AppLanguage lang;
  std::wstring code; // e.g. "ja-JP"
  std::wstring name; // e.g. "日本語"
  bool isExternal;   // true = 从文件加载, false = 内置
  bool isRtl;        // 阿拉伯语等RTL语言
};

// 获取可用语言列表（内置 + 外部）
const std::vector<LanguageInfo> &GetAvailableLanguages();
// 初始化时加载外部语言文件
void LoadExternalLanguages();
// 检查当前语言是否RTL
bool IsRtlLanguage();
