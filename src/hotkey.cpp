...
bool RegisterHotkey(HWND hwnd) {
    if (g_isHotkeyEnabled) {
        ...
        if (RegisterHotKey(hwnd, ID_HOTKEY_TOGGLE, g_hotkeyModifiers, g_hotkeyVirtualKey)) {
            // 快捷键注册成功
            return true;
        } else {
            // 注册失败，快捷键冲突
...
bool RegisterQuickPasteHotkeys(HWND hwnd) {
    ...
    for (int i = 0; i < 9; i++) {
        if (!RegisterHotKey(hwnd, ID_HOTKEY_PASTE_1 + i, g_quickPasteModifiers, '1' + i)) {
            allSuccess = false;
        }
    }
    return allSuccess;
...
std::wstring GetQuickPasteModifierText() {
    std::wstring text;
    if (g_quickPasteModifiers & MOD_CONTROL) text += L"Ctrl+";
    if (g_quickPasteModifiers & MOD_ALT) text += L"Alt+";
    if (g_quickPasteModifiers & MOD_SHIFT) text += L"Shift+";
    if (g_quickPasteModifiers & MOD_WIN) text += L"Win+";
    return text;
}
